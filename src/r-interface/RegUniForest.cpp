          					 ...
          					 List& param_r)
{
  ...
  // create data objects  
  RLT_REG_DATA REG_DATA(X, Y, Ncat, obsweight, varprob);
  ...
  if (Prediction.n_elem > 0)
  {
    ...
    ReturnList["Error"] = mean(square(Prediction(valid) - Y(valid)));
  }
...
#pragma omp parallel num_threads(usecores)
  ...
  if (var_mode == 1)
  {
    ReturnList["Variance"] = compute_matched_variance(PredAll);
  }
  else if (var_mode == 2)
  {
    // infinitesimal jackknife variance
    ReturnList["Variance"] = compute_ij_variance(PredAll, ObsTrack);
  }
  else if (var_mode == 3)
  {
    // jackknife-after-bootstrap variance
    ReturnList["Variance"] = compute_jack_variance(PredAll, ObsTrack);
  }
