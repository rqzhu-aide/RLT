...
void Surv_Uni_Split_Cont(Split_Class& TempSplit,
                        ...
                        vec& All_Risk,
                        vec& Temp_Vec,
                        const vec& obs_weight,                        int split_rule,
                        int nsplit,
                        double alpha,
                        bool useobsweight,
                        Rand& rngl)
                                ...
                                Rand& rngl)
{
    ...
    if (nsplit == 0) // best split  
    {
      ...
      for (size_t i = lowindex; i <= highindex; i++)
      {
        ...
        double score;
        if (useobsweight)
          score = CoxGrad_W(Pseudo_X, z_eta, obs_weight);
        else
          score = CoxGrad(Pseudo_X, z_eta);

        if (score > TempSplit.score)
        {
          TempSplit.value = (x(indices(i)) + x(indices(i+1)))/2;
          TempSplit.score = score;
...
//Weighted Cox gradient split rule
                 ...
                 const vec& obs_weight)
{
  ...
  //Calculate weighted score: imp = sum(w_i * z_i * PseudoX_i)
  double imp = 0;
  for(size_t i = 0; i<Pseudo_X.n_elem; i++){
    imp += obs_weight(i) * z_eta(i) * Pseudo_X(i);
  }
  return(imp*imp);
}
