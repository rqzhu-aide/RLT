                                      ...
                                      const size_t j)
{
  ...
  if (OneTree.SplitVar(Node, 0) == -1)
  {
    ...
    // BUG FIX: was recursing into Find_Terminal_Node_Comb (non-shuffle version),
    // which meant variable shuffling did not propagate past the first split node
    // that uses variable j. Must recurse into Find_Terminal_Node_Comb_ShuffleJ
    // to maintain the shuffled variable through all tree levels, consistent with
    // the non-Comb version Find_Terminal_Node_ShuffleJ.
    if (left_proxy.n_elem > 0)
    {
      Find_Terminal_Node_Comb_ShuffleJ(OneTree.LeftNode(Node), 
                              OneTree, 
                              X, 
                              Ncat, 
                              left_proxy, 
    ...
    if (proxy_id.n_elem > 0)
    {
      Find_Terminal_Node_Comb_ShuffleJ(OneTree.RightNode(Node), 
                              OneTree, 
                              X, 
                              Ncat, 
                              proxy_id, 
