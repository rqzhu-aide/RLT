                               ...
                               Rand& rngl)
{
  ...
  size_t comb_size = std::min((size_t)P, (size_t)Param.linear_comb);
  ...
  if (N >= switch_size)
  {
    ...
    // Determine which variables pass the threshold
    double threshold = Param.embed_threshold;
    
    size_t comb_valid = 0;
    for (size_t j = 0; j < comb_size; j++)
    {
      if (vi_embed(j) >= threshold * best_vi)
        comb_valid++;
      else
        break;
    }
    ...
    comb_valid = std::max(comb_valid, (size_t)1);
    
    // Protection list = the variables that passed the threshold
    var_protect = unique(join_cols(var_protect, var_id.subvec(0, comb_valid - 1)));
    
    // Check if sampled variable is categorical
    size_t sampled_idx = rngl.rand_sizet(0, comb_valid - 1);
    
    if (Cla_DATA.Ncat(var_id(sampled_idx)) > 1)
    {
    ...
    if (n_cont == 1) // single continuous split
    {
  ...
  {
    // ============================================================
    // Tier 2: Marginal Screen — univariate Gini screening, no protection/muting
    // ============================================================
    ...
    // Score each variable by its best univariate split (Gini reduction)
